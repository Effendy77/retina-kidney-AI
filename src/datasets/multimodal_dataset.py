import os
import pandas as pd
from PIL import Image
import torch
from torch.utils.data import Dataset
import torchvision.transforms as T


class MultimodalKidneyDataset(Dataset):
    """
    Loads:
      - fundus_image (RGB)
      - vessel_mask (1-channel)
      - tabular features
      - eGFR value (regression target)
    """

    def __init__(self, csv_path, image_root, mask_root):
        super().__init__()

        self.df = pd.read_csv(csv_path)

        self.image_root = image_root
        self.mask_root = mask_root

        # MUST MATCH COLUMNS IN YOUR CSV
        self.tabular_features = [
            "age",
            "sex",
            "diabetes",
            "hypertension",
            "qrisk3",
            "dm_htn_combined"
        ]

        # Transforms
        self.img_transform = T.Compose([
            T.Resize((224, 224)),
            T.ToTensor(),
            # Normalize with ImageNet stats (suitable for ViT/RETFound)
            T.Normalize(mean=[0.485, 0.456, 0.406],
                        std=[0.229, 0.224, 0.225]),
        ])

        self.mask_transform = T.Compose([
            T.Resize((224, 224)),
            T.ToTensor(),   # 1-channel mask
        ])

    def __len__(self):
        return len(self.df)

    def __getitem__(self, idx):
        row = self.df.iloc[idx]

        # Full paths already in CSV
        img_path = row["fundus_image"]
        mask_path = row["vessel_mask"]

        # If CSV contains relative paths, join with provided roots
        if not os.path.isabs(img_path):
            img_path = os.path.join(self.image_root, img_path)
        if not os.path.isabs(mask_path):
            mask_path = os.path.join(self.mask_root, mask_path)

        # Load fundus image
        img = Image.open(img_path).convert("RGB")
        img = self.img_transform(img)

        # Load vessel mask
        mask = Image.open(mask_path).convert("L")
        mask = self.mask_transform(mask)
        # Ensure binary mask and float dtype [0,1], shape [1,H,W]
        mask = (mask > 0.5).float()

        # Tabular features
        tab_series = row[self.tabular_features].astype(float).fillna(0.0)
        tab = torch.tensor(tab_series.values, dtype=torch.float32)

        # eGFR target
        target = torch.tensor(row["egfr"], dtype=torch.float32)

        return {
            "image": img,
            "mask": mask,
            "tabular": tab,
            "egfr": target,
        }
